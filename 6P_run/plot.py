...
r20, r21, r22 = [],[],[]
# AE wavefunctions
file1 = open('./P_Pae.dat', 'r')
# PS wavefunctions
file2 = open('./P_Pus.dat', 'r')
...
#axes[0,0].set_title("6p WF: ref = "+str(ref)+", rc = "+str(rc))
axes[0,0].set_title("6p: Wave Function")
...
#axes[0,1].set_title("6p LG: ref = "+str(ref)+", rc = "+str(rc))
axes[0,1].set_title("6p: Logarithmic Derivative")
...
axes[1,0].legend(('p orbital',), loc='upper right')
axes[1,0].set_title("6p: Error in Energy")

## plot Fourier components:
r10, r11, r12 = [], [], []
ffile1 = open('./P_Qbar.dat', 'r')
lines1 = ffile1.readlines()
for line in lines1:
    r10.append(float(line.split()[0]))
    r11.append(float(line.split()[1]))
...
axes[1,1].legend(('L = 0', 'L = 2'), loc='lower right')
axes[1,1].set_title("6p: Fourier Components")

fig.suptitle(f'6p orbital, ref = {ref}, rc = {rc}', fontsize=12)
plt.tight_layout()
plt.subplots_adjust(top=0.90)
save="../6P_figures/6p_ref="+str(ref)+"_rc="+str(rc)+".eps"
